...
def ShowCategory(title, category, page_count, is_popular): # generate category submenu of given category
...
def PageEpisodes(show_title, show_url): # generate serie submenu of given show url
...
######################################################################################
# Returns a list of the episodes, each episode is a DirectoryObject containing the mirrors

@route(PREFIX + "/listepisodes")
def ListEpisodes(page_url):	# generate episode listing page
    ...
    return oc
######################################################################################
#generates PopupDirectoryObject for each mirror in a single episode
...
# Gets the video(s?) for the chosen episode/mirror.
@route(PREFIX + "/getparts")
def GetParts(ep_url, host_url):

    oc = ObjectContainer()
    page_data = HTML.ElementFromURL(ep_url)
    ep_title = page_data.xpath("//div[@id='top_block']/h1/text()")[0].strip()

    # get the parts for the first mirror on page -- currently not working
    ...
    # get the part for the selected mirror on page
    if len(oc) < 1: # if there are no parts this section runs.
        oc.add(VideoClipObject(
